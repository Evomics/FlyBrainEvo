...
process_cell_info_ref_comp <- function(data, orig_value, cell_info) {
  data %>%
...
Dsim_ref_comp <- process_cell_info_ref_comp(cell_info_Dsim, 'Dsim', cell_info_Dsim_to_DmelRef)

df_plot_Dsim_ref_comp <- Dsim_ref_comp %>%
  na.omit() %>%
  dplyr::select(Dsim_to_Dsim = Cluster.x, Dsim_to_Dmel =Cluster.y) %>%
  make_long(Dsim_to_Dsim, 
            Dsim_to_Dmel) 

ggplot(df_plot_Dsim_ref_comp, aes(x = x,                        
                         next_x = next_x,                                     
                         node = node,
                         next_node = next_node,        
                         fill = factor(node))) +
