...
sample_list <- c("Dmel", "Dsim","Dsec", "DsecNoni")
...
sample_cells <- sample(colnames(TrioBrain_DF.list[[1]]), 10000)
seurat_subset <- subset(TrioBrain_DF.list[[1]], cells = sample_cells)
test_ISub_DF <- ISub_DF_sp(Seurat_object=seurat_subset, n_iteration = 10, sample_name='test')
  
Dmel_ISub_DF <- ISub_DF_sp(Seurat_object=TrioBrain_DF.list[[1]], n_iteration = 10, sample_name=sample_list[1])
Dsim_ISub_DF <- ISub_DF_sp(Seurat_object=TrioBrain_DF.list[[2]], n_iteration = 10, sample_name=sample_list[2])
Dsec_ISub_DF <- ISub_DF_sp(Seurat_object=TrioBrain_DF.list[[3]], n_iteration = 10, sample_name=sample_list[3])
DsecNoni_ISub_DF <- ISub_DF_sp(Seurat_object=TrioBrain_DF.list[[4]], n_iteration = 10, sample_name=sample_list[4])

Trio_ISub_DF_list <- list(Dmel_ISub_DF, Dsim_ISub_DF, Dsec_ISub_DF, DsecNoni_ISub_DF)

Dmel_ISub_DF[["cluster_id"]] <- Idents(object = Dmel_ISub_DF)
Dsim_ISub_DF[["cluster_id"]] <- Idents(object = Dsim_ISub_DF)
Dsec_ISub_DF[["cluster_id"]] <- Idents(object = Dsec_ISub_DF)
DsecNoni_ISub_DF[["cluster_id"]] <- Idents(object = DsecNoni_ISub_DF)

#saveRDS(Trio_ISub_DF_list, file = "Processed_Data/Trio_ISub_DF_list.rds")
Trio_ISub_DF_list <- readRDS(file = "Processed_Data/Trio_ISub_DF_list.rds")

#colnames(Trio_ISub_DF_list[[4]]@meta.data) <- gsub("clustser_id", "cluster_id", colnames(Trio_ISub_DF_list[[4]]@meta.data))

## Annotation ##

Trio_ISub_DF_marker_list <- lapply(X = Trio_ISub_DF_list, FUN = FindAllMarkers, only.pos = TRUE, min.pct = 0.15, 
                                   logfc.threshold = 0.25, test.use = "MAST")

#saveRDS(Trio_ISub_DF_marker_list, file = "Processed_Data/Trio_ISub_DF_marker_list.rds")
Trio_ISub_DF_marker_list <- readRDS(file = "Processed_Data/Trio_ISub_DF_marker_list.rds")
...
Trio_ISub_DF_sig_marker_list <- lapply(X=Trio_ISub_DF_marker_list, FUN=sig_markers)

dataset <- c("Dmel","Dsim","Dsec","DsecNoni")
...
Trio_ISub_DF_list_labeled <- Trio_ISub_DF_list
...
for (i in 1:4) {
  ...
  seurat_anno <- Trio_ISub_DF_list_labeled[[i]]
  ...
  df_glial_markers <- Trio_ISub_DF_sig_marker_list[[i]] %>%
    na.omit() %>%
  ...
  for (ct in glial_celltypes) {
    ...
    out_annotate_celltype <- annotate_celltype_individual(seurat_object=Trio_ISub_DF_list_labeled[[i]],
                                               cell_type=ct, df_markers = df_glial_markers, metadata=ISub_DF_metadata_summary)
    
    Trio_ISub_DF_list_labeled[[i]] <- out_annotate_celltype[[1]]
...
for (i in 1:4) {
  ...
  df_KC_markers <- Trio_ISub_DF_sig_marker_list[[i]] %>%
    na.omit() %>%
...
for (i in 1:4) {
  ...
  df_MA_markers <- Trio_ISub_DF_sig_marker_list[[i]] %>%
    na.omit() %>%
...
for (i in 1:4) {
  ...
  df_KD_markers <- Trio_ISub_DF_sig_marker_list[[i]] %>%
    na.omit() %>%
    na.omit() %>%
...
for (i in 1:4) {
  ...
  np_markers <- get_marker_nps(seurat_anno, Trio_ISub_DF_sig_marker_list[[i]], frac_threshold=0.05)
  
  df_np_markers <- Trio_ISub_DF_sig_marker_list[[i]] %>%
    na.omit() %>%
...
for (i in 1:4) {
  ...
  df_NT_markers <- Trio_ISub_DF_sig_marker_list[[i]] %>%
    na.omit() %>%
...
Trio_ISub_DF_list_labeled <- lapply(X=Trio_ISub_DF_list_labeled, FUN=add_cluster_label)

#saveRDS(Trio_ISub_DF_list_labeled, file = "Processed_Data/Trio_ISub_DF_list_labeled.rds")
Trio_ISub_DF_list_labeled <- readRDS(file = "Processed_Data/Trio_ISub_DF_list_labeled.rds")

DimPlot(subset(Trio_ISub_DF_list_labeled[[1]], ClusterLabel != 'Doublets'), label=T, reduction = 'tsne', repel=T, group.by='ClusterLabel') + NoLegend() + NoAxes()
DimPlot(subset(Trio_ISub_DF_list_labeled[[2]], ClusterLabel != 'Doublets'), label=T, reduction = 'tsne', repel=T, group.by='ClusterLabel') + NoLegend() + NoAxes()
...
# Combine the results
cell_info_join_Trio <- dplyr::bind_rows(cell_info_join_Dmel, cell_info_join_Dsim, cell_info_join_Dsec, cell_info_join_DsecNoni)

save(cell_info_join_Trio, file="Processed_Data/cell_info_join_Trio.RData")
