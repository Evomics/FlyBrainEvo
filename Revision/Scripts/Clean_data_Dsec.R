...
sc_Dsec1 <- load10X('cellranger/brain/Dsec.count/run_count_dsec_brain_1/outs/')
...
sc_Dsec2 <- load10X('cellranger/brain/Dsec.count/run_count_dsec_brain_2/outs/')
...
sc_Dsec3 <- load10X('cellranger/brain/Dsec.count/run_count_dsec_brain_3/outs/')
...
sc_Dsec4 <- load10X('cellranger/brain/Dsec.count/run_count_dsec_brain_4/outs/')
...
sc_Dsec5 <- load10X('cellranger/brain/Dsec.count/run_count_dsec_brain_5/outs/')
...
sc_Dsec6 <- load10X('cellranger/brain/Dsec.count/run_count_dsec_brain_6/outs/')
...
bcmvn_Dsec_decon_combined <- find.pK(sweep.stats_Dsec_decon_combined)  ## pk=0.14
...
Dsec_decon_combined <- doubletFinder_v3_SCT(Dsec_decon_combined, PCs = 1:50, pN = 0.25, pK = 0.14, nExp = nExp_poi.adj, reuse.pANN = F, sct = T)
...
Dsec_decon_combined_DF <- subset(Dsec_decon_combined, subset = DF.classifications_0.25_0.14_7607 == "Singlet")
...
DefaultAssay(Dsec_decon_combined) <- 'SCT'

FeaturePlot(Dsec_decon_combined, feature='Pka-R2', order=T)
DimPlot(Dsec_decon_combined, label=T)
#DimPlot(subset(Dsec_decon_combined, idents =c(6,7,13,16)), label=T)

Dsec_celltype_KC_doublet <- subset(Dsec_decon_combined, idents = c(7,8,12))
...
Dsec_celltype_KC_DF <- subset(Dsec_celltype_KC_doublet, subset = DF.classifications_0.25_0.14_7607 == "Singlet")
...
FeaturePlot(object=Dsec_celltype_KC_DF, feature="rn")
