...
### loop for subclustering 

for (cl in 10:20) {
  
  subset <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = cl, resolution = 0.1)
  DefaultAssay(subset[[1]]) <- "SCT"
  subset_seurat <- DietSeurat(subset[[1]], assays = c("RNA","SCT"), scale.data = T, dimreducs = c("umap", "tsne"))
  subset_seurat <- PrepSCTFindMarkers(subset_seurat, assay = "SCT", verbose = TRUE)
  
  subset_markers <- FindAllMarkers(subset_seurat, only.pos = F, min.pct = 0.15, 
                                     logfc.threshold = 0.25, test.use = "MAST")
  
  save(subset_markers, subset_seurat,file=glue::glue("Processed_Data/Cluster{cl}.RData"))
  
  top24_subset <- subset_markers %>% 
    group_by(cluster) %>% 
    #dplyr::filter(pct.2 < 0.2) %>%
    top_n(n = 24, wt = -p_val) %>% top_n(n = 24, wt = avg_log2FC)
  
  for (i in unique(top24_subset$cluster)) {
    
    top24_subset_sub <- top24_subset %>%
      dplyr::filter(cluster == i)
    
    VlnPlot(subset_seurat, features = top24_subset_sub$gene, 
            pt.size = 0, ncol=3, assay = "SCT")
    
    ggsave(glue::glue("Plots/Markers/Subcluster/TrioBrain_markers_Cluster{cl}_subclusters{i}.png"), width=24, height = 24)
...
Cluster0 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 0, resolution = 0.1)
...
DimPlot(Cluster0_seurat)

Cluster0_0 <- Subclustering(Seurat_object=Cluster0_seurat, cluster = 0, resolution = 0.1)
DefaultAssay(Cluster0_0[[1]]) <- "SCT"
Cluster0_0_seurat <- DietSeurat(Cluster0_0[[1]], assays = c("RNA","SCT"), scale.data = T, dimreducs = c("umap", "tsne"))
Cluster0_0_seurat <- PrepSCTFindMarkers(Cluster0_0_seurat, assay = "SCT", verbose = TRUE)
Cluster0_0_markers <- FindAllMarkers(Cluster0_0_seurat, only.pos = F, min.pct = 0.15, 
                                   logfc.threshold = 0.25, test.use = "MAST")

FeaturePlot(Cluster0_0_seurat, feature='Sox21b')

Cluster0_0_0 <- Subclustering(Seurat_object=Cluster0_0_seurat, cluster = 0, resolution = 0.1)


load("Processed_Data/Cluster1.RData")
DimPlot(Cluster1_seurat)
Cluster1_0 <- Subclustering(Seurat_object=Cluster1_seurat, cluster = 0, resolution = 0.1)
Cluster1_0_0 <- Subclustering(Seurat_object=Cluster1_0[[1]], cluster = 0, resolution = 0.1)

subset(Cluster1_0[[1]], idents=2)
...
ggsave(glue::glue("Plots/subcluster_marker/TrioBrain_DF_KD_cluster_intersect_K50_Cluster0.png"), width=20, height =20)
...
FeaturePlot(TrioBrain.integrated_slim, features=c("FMRFa"))

## Cluster 1 ##

Cluster1 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 1, resolution = 0.1)
...
subset(TrioBrain.integrated_slim, ident = 'ort')

Cluster1[[3]]

FeaturePlot(TrioBrain.integrated_slim, features = c("Oaz", "Proc"), order=T)
...
Cluster2 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 2, resolution = 0.1)
...
subset(TrioBrain.integrated_slim, ident = 'Dh31(I)')
subset(TrioBrain.integrated_slim, ident = 'Poxn')

subset(Cluster2_seurat, ident = 7)
subset(TrioBrain.integrated_slim, ident = 'fru')
...
Cluster6 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'Cluster6', resolution = 0.1, npc=10)
...
Cluster7 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'Cluster7', resolution = 0.1)
...
FeaturePlot(TrioBrain.integrated_slim, feature="Mip")
...
FeaturePlot(subset(TrioBrain.integrated_slim, idents = "MON"), feature = c("ple", "Tdc2", "Tbh", "SerT"), pt.size=0.01)
...
Cluster11 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'fru', resolution = 0.1, npc=10)
...
Cluster12 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'sNPF', resolution = 0.1, npc=10)
...
FeaturePlot(subset(TrioBrain.integrated_slim, ident="sNPF"), feature=c("VGlut"))
FeaturePlot(subset(TrioBrain.integrated_slim, ident="sNPF"), feature=c("sNPF"))
FeaturePlot(subset(TrioBrain.integrated_slim, ident="sNPF"), feature=c("CCHa2"))
FeaturePlot(subset(TrioBrain.integrated_slim, ident="sNPF"), feature=c("Nos"))

subset(Cluster0_seurat, ident = 5)
subset(Cluster12_seurat, ident = 4)

FeaturePlot(TrioBrain.integrated_slim, feature=c("FMRFa"), reduction = 'tsne')
FeaturePlot(TrioBrain.integrated_slim, feature=c("sNPF"), reduction = 'tsne')
FeaturePlot(subset(TrioBrain.integrated_slim, ident="sNPF"), feature=c("VGlut"), reduction = 'tsne', order=T)
FeaturePlot(subset(TrioBrain.integrated_slim, ident="sNPF"), feature=c("CCHa2"), reduction = 'tsne', order=T)
FeaturePlot(subset(TrioBrain.integrated_slim, ident="sNPF"), feature=c("Nep1"), reduction = 'tsne', order=T)

VlnPlot(TrioBrain.integrated_slim, feature=c("sNPF", "FMRFa", "CCHa2"), raster=FALSE)
VlnPlot(TrioBrain.integrated_slim, feature=c("CCHa2"), raster=FALSE)

subset(Cluster12_seurat, ident = 3)
subset(TrioBrain.integrated_slim, ident = 'CCHa2')
subset(TrioBrain.integrated_slim, ident = 'Tk')
...
Cluster14 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'PRN', resolution = 0.1, npc=10)
...
Cluster15 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'Cluster15', resolution = 0.1, npc=50)
...
Cluster17 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'PEP', resolution = 0.1, npc=10)
...
Cluster19 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'Cluster19', resolution = 0.1, npc=10)
...
Cluster20 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'Clock', resolution = 0.1, npc=10)
...
FeaturePlot(subset(TrioBrain.integrated_slim, ident="Clock"), feature=c("Clk", "tim"), order=T)
FeaturePlot(subset(TrioBrain.integrated_slim, ident="Clock"), feature=c("pros", "Imp"), order=T)

subset(Cluster20_seurat, ident=3)

VlnPlot(TrioBrain.integrated_slim, feature='Dh44')
...
Cluster22 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'Cluster22', resolution = 0.1, npc=10)
...
Cluster23 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'Cluster23', resolution = 0.1, npc=10)
...
Cluster24 <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 'CTX', resolution = 0.1, npc=5)
...
FeaturePlot(subset(TrioBrain.integrated_slim, idents='fru'), feature = "fru")
FeaturePlot(subset(TrioBrain.integrated_slim, idents='sNPF'), feature = "sNPF")
FeaturePlot(subset(TrioBrain.integrated_slim, idents='Dh31(E)'), feature = "Dh31")
FeaturePlot(subset(TrioBrain.integrated_slim, idents='Clock'), feature = "Clk", order=T)
FeaturePlot(subset(TrioBrain.integrated_slim, idents='Clock'), feature = "tim", order=T)
...
KCab <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 3, resolution = 0.1)
...
FeaturePlot(TrioBrain.integrated_slim, feature=c("rn", "CG4829","mtt", "Dscam3","wake","Hr38"), pt.size= 0.01, order=T)
...
KCr <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 6, resolution = 0.2)
...
FeaturePlot(TrioBrain.integrated_slim, feature=c("rn", "CG14989","SKIP", "crb"), pt.size= 0.01, order=T)

## KCa'b' ##

KCapbp <- Subclustering(Seurat_object=TrioBrain.integrated_slim, cluster = 15, resolution = 0.2)
...
FeaturePlot(TrioBrain.integrated_slim, feature=c("ct", "CG14989","SKIP", "crb"), pt.size= 0.01, order=T)

DotPlot(TrioBrain.integrated_slim, feature=c("Oaz", "otp"))
...
FeaturePlot(TrioBrain.integrated_slim, feature = c("CAH1", "CAH2","CAH3"), order=T, ncol=3, raster=F)
...
TrioBrain.integrated_slim <- AddModuleScore(object = TrioBrain.integrated_slim, features = list(OPN_markers), name = 'OPN')

VlnPlot(TrioBrain.integrated_slim, features=c("OPN1"), pt.size=0) +
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5),
        axis.title.x = element_blank()) +
  NoLegend() 
...
FeaturePlot(TrioBrain.integrated_slim, feature = c("otp", "SiaT", "hth", "Oaz"), order=T, ncol=2, raster=F)
...
FeaturePlot(TrioBrain.integrated_slim, feature = c("nAChRalpha6", "CG42458", "Eaat1"), order=T, ncol=3, raster=F)
