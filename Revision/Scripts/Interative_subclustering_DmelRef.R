...
TrioBrain_DmelRef.integrated_slim<- readRDS(file = "Processed_Data/TrioBrain_DmelRef_DF.integrated_slim.rds")
#TrioBrain_DmelRef.integrated.markers <- readRDS(file = "Processed_Data/TrioBrain_DmelRef_DF.integrated.markers.rds")

df_TrioBrain_DmelRef.integrated_slim_metadata <- TrioBrain_DmelRef.integrated_slim@meta.data %>%
...
total_cell_number <- nrow(TrioBrain_DmelRef.integrated_slim@meta.data)

metadata_summary <- TrioBrain_DmelRef.integrated_slim@meta.data %>%
...
ISub <- function(Seurat_object, cluster, method='rpca', resolution = 0.1, npc=50, genelist=shared_genes_DmelRef, kweight=100) {
...
DefaultAssay(TrioBrain_DmelRef.integrated_slim) <- 'RNA'
TrioBrain_DmelRef.integrated_slimer <- DietSeurat(TrioBrain_DmelRef.integrated_slim, assays = c("RNA"))
DefaultAssay(TrioBrain_DmelRef.integrated_slim) <- 'SCT'

cluster_range_max <- max(as.numeric(as.vector(dplyr::filter(metadata_summary, percent > 0.5)$seurat_clusters)))

TrioBrain_DmelRef.integrated_slim_ISub_DF <- TrioBrain_DmelRef.integrated_slim
...
for (cl in cluster_range_max:0) {
  ...
  assign(name, ISub(TrioBrain_DmelRef.integrated_slimer, cluster = cl, resolution = 0.1))
  ...
  Idents(TrioBrain_DmelRef.integrated_slim_ISub_DF, cells = name_DF_cellid) <- "Doublets"
  ...
  if (n_cluster_pass >=2) {
    ...
    for (subB in df2$seurat_clusters) {
      ...
      Idents(TrioBrain_DmelRef.integrated_slim_ISub_DF, cells = name_subB_cellid) <- name_subB
    ...
    for (subC in df3$seurat_clusters) {
      ...
      Idents(TrioBrain_DmelRef.integrated_slim_ISub_DF, cells = name_subC_cellid) <- cl
    ...
    Idents(TrioBrain_DmelRef.integrated_slim_ISub_DF, cells = name_cellid) <- cl
  ...
  for (j in 1:n_remainders) {
    ...
    if (n_cluster_pass >=2) {
      ...
      for (subB in df2$seurat_clusters) {
        ...
        Idents(TrioBrain_DmelRef.integrated_slim_ISub_DF, cells = remainders_subB_cellid) <- remainders_subB
      ...
      for (subC in df3$seurat_clusters) {
        ...
        Idents(TrioBrain_DmelRef.integrated_slim_ISub_DF, cells = remainders_subC_cellid) <- remain_name
      ...
      Idents(TrioBrain_DmelRef.integrated_slim_ISub_DF, cells = remainders_cellid) <- remain_name
...
saveRDS(TrioBrain_DmelRef.integrated_slim_ISub_DF, file = "Processed_Data/TrioBrain_DmelRef.integrated_slim_ISub_DF.rds")

unique(Idents(TrioBrain_DmelRef.integrated_slim_ISub_DF))

TrioBrain_DmelRef.integrated_slim_ISub_DF[["CellType"]] <- Idents(object = TrioBrain_DmelRef.integrated_slim_ISub_DF)

TrioBrain_DmelRef.integrated_slim_ISub_DF_metadata <- TrioBrain_DmelRef.integrated_slim_ISub_DF@meta.data %>%
  dplyr::mutate(species=ifelse(orig.ident %in% c("Dmel_rep1", "Dmel_rep2", "Dmel_rep3", "Dmel_rep4", "Dmel_rep5", "Dmel_rep6"), "Dmel",
                               ifelse(orig.ident %in% c("Dsim_rep1", "Dsim_rep2", "Dsim_rep3", "Dsim_rep4", "Dsim_rep5", "Dsim_rep6"), "Dsim",
                                      ifelse(orig.ident %in% c("Dsec_rep1", "Dsec_rep2", "Dsec_rep3", "Dsec_rep4", "Dsec_rep5", "Dsec_rep6"), "Dsec",
                                             "DsecNoni")))) %>%
...
TrioBrain_DmelRef.integrated_slim_ISub_DF_metadata_summary_rep <- TrioBrain_DmelRef.integrated_slim_ISub_DF_metadata %>%
...
TrioBrain_DmelRef.integrated_slim_ISub_DF_metadata_summary <- TrioBrain_DmelRef.integrated_slim_ISub_DF_metadata_summary_rep %>%
  dplyr::group_by(species, CellType) %>%
  dplyr::summarise(percent_combined = sum(n)/sum(total)*100, sem=sd(percent)/sqrt(6)) %>%
  dplyr::ungroup()

TrioBrain_DmelRef.integrated_slim_ISub_DF_metadata_summary %>%
  ggplot(.) +
  ...
  geom_point(data=TrioBrain_DmelRef.integrated_slim_ISub_DF_metadata_summary_rep, size = 1, alpha =0.8,  
