...
sc_Dsim1 <- load10X('cellranger/brain/Dsim.count/run_count_dsim_brain_1/outs/')
...
sc_Dsim2 <- load10X('cellranger/brain/Dsim.count/run_count_dsim_brain_2/outs/')
...
sc_Dsim3 <- load10X('cellranger/brain/Dsim.count/run_count_dsim_brain_3/outs/')
...
sc_Dsim4 <- load10X('cellranger/brain/Dsim.count/run_count_dsim_brain_4/outs/')
...
sc_Dsim5 <- load10X('cellranger/brain/Dsim.count/run_count_dsim_brain_5/outs/')
...
sc_Dsim6 <- load10X('cellranger/brain/Dsim.count/run_count_dsim_brain_6/outs/')
...
bcmvn_Dsim_decon_combined <- find.pK(sweep.stats_Dsim_decon_combined)  ## pk=0.12
...
Dsim_decon_combined <- doubletFinder_v3_SCT(Dsim_decon_combined, PCs = 1:50, pN = 0.25, pK = 0.12, nExp = nExp_poi.adj, reuse.pANN = F, sct = T)

DimPlot(Dsim_decon_combined,
        pt.size = 0.01, label=F, reduction = "umap",
        group.by = "DF.classifications_0.25_0.12_6981" )+theme(aspect.ratio = 1)
...
Dsim_decon_combined_DF <- subset(Dsim_decon_combined, subset = DF.classifications_0.25_0.12_6981 == "Singlet")
...
DefaultAssay(Dsim_decon_combined) <- 'SCT'

FeaturePlot(Dsim_decon_combined, feature='Pka-R2', order=T)
DimPlot(Dsim_decon_combined, label=T)
DimPlot(subset(Dsim_decon_combined, idents =c(6,7,13,16)), label=T)

Dsim_celltype_KC_doublet <- subset(Dsim_decon_combined, idents = c(6,7,13,16))
...
Dsim_celltype_KC_DF <- subset(Dsim_celltype_KC_doublet, subset = DF.classifications_0.25_0.12_6981 == "Singlet")
...
FeaturePlot(object=Dsim_celltype_KC_DF, feature="rn")
